...
def evaluate(args):
    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        ...
        with tf.device('/gpu:0'):
            ...
            one_bin_off_loss = tf.reduce_mean(tf.cast(tf.less_equal(
                tf.abs(tf.cast(tf.argmax(logits[1], axis=1), tf.float64) - tf.cast(labels[:, 1], tf.float64)), 1),
                                                      tf.float32))
            ...
            binary_behavior_logits = tf.cast(
                [top1ind_bh[0], top1ind_bh[1], top1ind_bh[2], top1ind_bh[3], top1ind_bh[4], top1ind_bh[5]], tf.int32)
            # Cast to Boolean
            binary_behavior_predictions = tf.squeeze(tf.cast(binary_behavior_logits, tf.bool))
            # Group labels together
            binary_behavior_labels_logits = [labels[:, 2], labels[:, 3], labels[:, 4], labels[:, 5], labels[:, 6],
                                             labels[:, 7]]
            ...
            union_length = tf.reduce_sum(
                tf.cast(tf.logical_or(binary_behavior_labels, binary_behavior_predictions), tf.int32), axis=0)
            # Compute the intersection of the labels set and prediction set
            intersect_length = tf.reduce_sum(
                tf.cast(tf.logical_and(binary_behavior_labels, binary_behavior_predictions), tf.int32), axis=0)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            while step < args.num_batches and not coord.should_stop():
                ...
                top1_accuracy, top5_accuracy, top3_accuracy, urls_values, label_values, top5guesses_id, top5conf, top3guesses_cn, top3conf, top1guesses_bh, top1conf, obol_val, yval, zval, uval, ival = sess.run(
                    [top1acc, top5acc_id, top3acc_cn, urls, labels, top5ind_id, top5val_id, top3ind_cn, top3val_cn,
                    ...
                    out_file.write(predictions_format_str % (step * args.batch_size + i + 1, urls_values[i],
                                                             ...
                                                             '[' + ', '.join('%d' % np.asscalar(item) for item in
                                                                             [top1guesses_bh[0][i],
                                                                              ...
                                                                              top1guesses_bh[5][i]]) + ']',
                                                             '[' + ', '.join('%.3f' % np.asscalar(item) for item in
                                                                             [top1conf[0][i], top1conf[1][i],
                                                                              top1conf[2][i], top1conf[3][i],
                                                                              top1conf[4][i], top1conf[5][i]]) + ']'))
                    out_file.flush()
                ...
                step, '[' + ', '.join('%.3f' % (item / (step + 1.0)) for item in true_predictions_count) + ']',
                true_top5_predictions_count / (step + 1.0), true_top3_predictions_count / (step + 1.0),
                obol_val / (step + 1.0), accv_all / total_examples, prcv_all / total_examples,
                recv_all / total_examples))
                sys.stdout.flush()
                step += 1
...
def main():
    ...
    parser.add_argument('--load_size', nargs=2, default=[256, 256], type=int, action='store',
                        help='The width and height of images for loading from disk')
    parser.add_argument('--crop_size', nargs=2, default=[224, 224], type=int, action='store',
    ...
    parser.add_argument('--num_classes', default=[48, 12, 2, 2, 2, 2, 2, 2], type=int, nargs='+',
    ...
    parser.add_argument('--path_prefix', default='/project/EvolvingAI/mnorouzz/Serengiti/resized', action='store',
                        help='The prefix address for images')
    parser.add_argument('--delimiter', default=',', action='store', help='Delimiter for the input files')
    parser.add_argument('--data_info', default='gold_expert_info.csv', action='store',
    ...
    parser.add_argument('save_predictions', default=None, action='store',
                        help='Save predictions of the networks along with their confidence in the specified file')
